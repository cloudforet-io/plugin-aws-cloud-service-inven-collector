...
from spaceone.inventory.connector.aws_lightsail_connector.schema.data import (
    Instance,
    Disk,
    Bucket,
    DiskSnapshot,
...
from spaceone.inventory.connector.aws_lightsail_connector.schema.resource import (
    ...
    DiskResource,
    DiskResponse,
    DiskSnapshotResource,
    DiskSnapshotResponse,
    BucketResource,
    BucketResponse,
    StaticIPResource,
    StaticIPResponse,
    RelationDatabaseResource,
...
from spaceone.inventory.connector.aws_lightsail_connector.schema.service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class LightsailConnector(SchematicAWSConnector):
    ...
    def get_resources(self):
        _LOGGER.debug(
            f"[get_resources][account_id: {self.account_id}] START: Lightsail"
        )
        resources = []
        ...
        for region_name in self.region_names:
            try:
                if (region_name in EXCLUDE_REGION) or (
                    region_name not in ALLOWED_REGION
                ):
                    continue
                ...
                for collect_resource in collect_resources:
                    ...
                    if (
                        collect_resource["request_method"] == self.request_domain_data
                    ) or (
                        collect_resource["request_method"]
                        == self.request_distribution_data
                    ):
                        # Global resource query interface to us-east-1 region
        ...
        _LOGGER.debug(
            f"[get_resources][account_id: {self.account_id}] FINISHED: Lightsail ({time.time() - start_time} sec)"
        )
        return resources
    ...
    def request_buckets_data(self, region_name):
        ...
        for data in self.client.get_buckets().get("buckets", []):
            try:
                ...
                data.update({"object_count": count, "object_total_size": size})
                ...
                yield {
                    ...
                    "account": self.account_id,
                    "tags": self.convert_tags_to_dict_type(
                        data.get("tags", []), key="key", value="value"
                    ),
                }
    ...
    def _get_bucket_metric_data(self, bucket_name, metric_name):
        ...
        return (
            response.get("metricData")[0].get("average", 0)
            if len(response.get("metricData")) > 0
            else 0
        )
