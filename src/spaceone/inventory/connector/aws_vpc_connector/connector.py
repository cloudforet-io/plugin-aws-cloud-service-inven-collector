...
from spaceone.inventory.connector.aws_vpc_connector.schema.data import (
    ...
    NetworkACL,
    NetworkACLEntries,
    NetworkACLTotalEntries,
    TransitGateway,
    CustomerGateway,
    VPNGateway,
    VPNConnection,
)
from spaceone.inventory.connector.aws_vpc_connector.schema.resource import (
    VPCResource,
    VPCResponse,
    SubnetResource,
    SubnetResponse,
    ...
    NATGatewayResource,
    NATGatewayResponse,
    PeeringConnectionResource,
    PeeringConnectionResponse,
    NetworkACLResource,
    NetworkACLResponse,
    RouteTableResource,
    RouteTableResponse,
...
from spaceone.inventory.connector.aws_vpc_connector.schema.service_type import (
    CLOUD_SERVICE_TYPES,
)
from spaceone.inventory.libs.connector import SchematicAWSConnector

_LOGGER = logging.getLogger(__name__)
PROTOCOL_NUMBER_INFO = {
    "0": "HOPOPT",
    "1": "ICMP",
    "2": "IGMP",
    "3": "GGP",
...
class VPCConnector(SchematicAWSConnector):
    ...
    def get_resources(self):
        ...
        _LOGGER.debug(
            f"[get_resources][account_id: {self.account_id}] FINISHED: VPC ({time.time() - start_time} sec)"
        )
        return resources
    ...
    def request_vpc_data(self, region_name) -> List[VPC]:
        ...
        for vpc in self.vpcs:
            try:
                route_tables = self._match_vpc_object(
                    self.route_tables, vpc.get("VpcId")
                )

                vpc.update(
                    {
                        "cidr_blocks": self._get_associated_cidr_blocks(
                            vpc.get("CidrBlockAssociationSet")
                        ),
                        ...
                        "subnets": self._match_vpc_object(
                            self.subnets, vpc.get("VpcId")
                        ),
                        "route_tables": route_tables,
                        "main_route_table_id": self._get_main_route_table(route_tables),
                        "main_network_acl_id": self._get_main_network_acl(
                            self._match_vpc_object(self.network_acls, vpc.get("VpcId"))
                        ),
                        "endpoints": self._match_vpc_object(
                            self.endpoints, vpc.get("VpcId")
                        ),
                        "peering_connections": self._match_vpc_peering_connection(
                            vpc.get("VpcId")
                        ),
                        "nat_gateways": self._match_vpc_object(
                            self.nat_gateways, vpc.get("VpcId")
                        ),
                        "transit_gateway": self._match_transit_gateway(
                            vpc.get("VpcId")
                        ),
                        "vpn_gateway": self._match_vpn_gateway(vpc.get("VpcId")),
                        "region_name": region_name,
                        "internet_gateway": self._match_internet_gateway(
                            vpc.get("VpcId")
                        ),
                        "enable_dns_support": self.describe_vpc_attribute(
                            vpc.get("VpcId"), "enableDnsSupport"
                        ),
                        "enable_dns_hostnames": self.describe_vpc_attribute(
                            vpc.get("VpcId"), "enableDnsHostnames"
                        ),
                        "name": self._get_name_from_tags(vpc.get("Tags", [])),
                        "cloudtrail": self.set_cloudtrail(
                            region_name, cloudtrail_resource_type, vpc["VpcId"]
                        ),
                    }
                )
    ...
    def request_peering_connection_data(self, region_name):
        ...
        if self.include_default:
            ...
            peerx_ids = [
                _peerx.get("VpcPeeringConnectionId")
                for _peerx in accepter_response.get("VpcPeeringConnections", [])
            ]
                ...
                ngw.update(
                    {
                        ...
                        "cloudtrail": self.set_cloudtrail(
                            region_name, cloudtrail_resource_type, ngw["NatGatewayId"]
                        ),
    ...
    def request_network_acl_data(self, region_name):
        ...
        for nacl in response.get("NetworkAcls", []):
            try:
                inbound_rules, outbound_rules, total_rules = self._get_rules_from_rules(
                    nacl.get("Entries", [])
                )
                ...
                nacl.update(
                    {
                        ...
                        "region_name": region_name,
                        "cloudtrail": self.set_cloudtrail(
                            region_name, cloudtrail_resource_type, nacl["NetworkAclId"]
                        ),
            ...
            try:
                endp.update(
                    {
                        ...
                        "region_name": region_name,
                        "cloudtrail": self.set_cloudtrail(
                            region_name, cloudtrail_resource_type, endp["VpcEndpointId"]
                        ),
                ...
                yield {
                    ...
                    "launched_at": self.datetime_to_iso8601(
                        endpoint_vo.creation_timestamp
                    ),
                    "account": self.account_id,
    ...
    def request_egress_only_internet_gateway_data(self, region_name):
        ...
        for eoigw in response.get("EgressOnlyInternetGateways", []):
            try:
                ...
                self.egress_only_internet_gateways.append(
                    egress_only_internet_gateway_vo
                )
                yield {
    ...
    def request_route_table_data(self, region_name):
        ...
        for rt in response.get("RouteTables", []):
            try:
                subnet_associations, edge_associations, main = self._get_association(
                    rt.get("Associations", [])
                )
                ...
                rt.update(
                    {
                        ...
                        "region_name": region_name,
                        "cloudtrail": self.set_cloudtrail(
                            region_name, cloudtrail_resource_type, rt["RouteTableId"]
                        ),
    ...
    def request_subnet_data(self, region_name):
        ...
        for subnet in response.get("Subnets", []):
            try:
                subnet.update(
                    {
                        "region_name": region_name,
                        "cloudtrail": self.set_cloudtrail(
                            region_name, cloudtrail_resource_type, subnet["SubnetId"]
                        ),
                ...
                if match_nat_gateways := self._match_nat_gateways(
                    subnet.get("SubnetId")
                ):
    ...
    def request_customer_gateway_data(self, region_name):
        ...
        for customer_gateway in response.get("CustomerGateways", []):
            try:
                ...
                _match_vpn_conn = self._match_vpn_connection(
                    "customer_gateway", customer_gateway.get("CustomerGatewayId")
                )

                if len(_match_vpn_conn) > 0:
                    customer_gateway.update({"vpn_connection": _match_vpn_conn[0]})
    ...
    def request_vpn_gateway_data(self, region_name):
        ...
        for vpn_gateway in response.get("VpnGateways", []):
            try:
                ...
                _match_vpn_conn = self._match_vpn_connection(
                    "vpn_gateway", vpn_gateway.get("VpnGatewayId")
                )

                if len(_match_vpn_conn) > 0:
                    vpn_gateway.update({"vpn_connection": _match_vpn_conn[0]})
    ...
    def _get_rules_from_rules(self, entries):
        ...
        for entry in entries:
            entry.update(
                {
                    "protocol_display": PROTOCOL_NUMBER_INFO.get(
    ...
    def _match_vpc_peering_connection(self, vpc_id):
        ...
        for peercon in self.peering_connections:
            if (
                getattr(peercon, "accepter_vpc_info", None) is not None
                and peercon.accepter_vpc_info.vpc_id == vpc_id
            ):
                match_peering_connections.append(peercon)
            elif (
                getattr(peercon, "requester_vpc_info", None) is not None
                and peercon.requester_vpc_info.vpc_id == vpc_id
            ):
                match_peering_connections.append(peercon)
    ...
    def _match_vpn_connection(self, resource_type, resource_id):
        ...
        for vpnconn in self.vpn_connections:
            if target_resource_id := getattr(
                vpnconn, resource_type_map.get(resource_type, ""), None
            ):
    ...
    def _get_associated_cidr_blocks(self, cidr_blocks):
        return [cidr for cidr in cidr_blocks if self._is_associated_cidr_block(cidr)]

    @staticmethod
    def _is_associated_cidr_block(cidr_block):
        if block_state := cidr_block.get("CidrBlockState"):
            return block_state.get("State") == "associated"
        return False
