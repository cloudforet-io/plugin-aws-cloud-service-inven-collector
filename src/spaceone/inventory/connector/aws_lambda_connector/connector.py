...
from spaceone.inventory.connector.aws_lambda_connector.schema.resource import (
    LambdaFunctionResponse,
    LambdaFunctionResource,
    LambdaLayerResource,
    LambdaLayerResponse,
)
from spaceone.inventory.connector.aws_lambda_connector.schema.service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class LambdaConnector(SchematicAWSConnector):
    ...
    def get_resources(self):
        ...
        _LOGGER.debug(
            f"[get_resources][account_id: {self.account_id}] FINISHED: Lambda ({time.time() - start_time} sec)"
        )
        return resources
        ...
        for data in response_iterator:
            for raw in data.get("Functions", []):
                try:
                    ...
                    func.cloudwatch = self.set_cloudwatch(
                        cloudwatch_namespace,
                        cloudwatch_dimension_name,
                        raw["FunctionName"],
                        region_name,
                    )
                    func.cloudtrail = self.set_cloudtrail(
                        region_name, cloudtrail_resource_type, func.name
                    )
            ...
            for raw in data["Layers"]:
                try:
                    ...
                    if "Version" in latest_matching_version:
                        raw.update({"version": latest_matching_version.get("Version")})

                    raw.update(
                        {
                            "region_name": region_name,
                            "cloudtrail": self.set_cloudtrail(
                                region_name, cloudtrail_resource_type, raw["LayerArn"]
                            ),
                        }
                    )
                    ...
                    yield {
                        ...
                        "launched_at": self.datetime_to_iso8601(
                            layer_vo.latest_matching_version.created_date
                        ),
                        "account": self.account_id,
                    }
