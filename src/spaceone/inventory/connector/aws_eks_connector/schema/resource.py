...
cluster_base = ItemDynamicLayout.set_fields(
    "Cluster",
    fields=[
...
cluster_node_groups = TableDynamicLayout.set_fields(
    "Node Groups",
    "data.node_groups",
    fields=[
        ...
        ListDyField.data_source(
            "Instance Types", "instance_types", options={"delimiter": "<br>"}
        ),
        ListDyField.data_source("Subnets", "subnets", options={"delimiter": "<br>"}),
        TextDyField.data_source("Disk Size", "disk_size"),
        ListDyField.data_source(
            "Health Issue", "instance_types", options={"delimiter": "<br>"}
        ),
    ],
)

cluster_addons = TableDynamicLayout.set_fields(
    "Add Ons",
    "data.add_ons",
    fields=[
        TextDyField.data_source("Addon Name", "add_on_name"),
        EnumDyField.data_source(
            "Status",
            "status",
            default_state={
        ...
        TextDyField.data_source("Add on Arn", "arn"),
        ListDyField.data_source(
            "EKS Pod Identity", "eks_pod_identity", options={"delimiter": "<br>"}
        ),
        TextDyField.data_source("IAM role for service account", "irsa"),
    ],
)
cluster_logging = TableDynamicLayout.set_fields(
    "Cluster Logging",
    "data.logging.cluster_logging",
    fields=[
        ListDyField.data_source("Types", "types", options={"delimiter": "<br>"}),
        EnumDyField.data_source(
            "Enabled",
            "enabled",
...
cluster_update = TableDynamicLayout.set_fields(
    "Updates",
    "data.updates",
    fields=[
        ...
        ListDyField.data_source(
            "Error", "errors.error_code", options={"delimiter": "<br>"}
        ),
    ],
)

cluster_metadata = CloudServiceMeta.set_layouts(
    layouts=[cluster_base, cluster_node_groups, cluster_addons, cluster_logging]
)
...
node_group_base = ItemDynamicLayout.set_fields(
    ...
    fields=[
        ...
        ListDyField.data_source(
            "Instance Types", "data.instance_types", options={"delimiter": "<br>"}
        ),
        ListDyField.data_source(
            "Subnets", "data.subnets", options={"delimiter": "<br>"}
        ),
        ListDyField.data_source(
            "Source Security Group",
            "data.remote_access.source_security_groups",
...
node_group_scaling_config = ItemDynamicLayout.set_fields(
    "Scaling Config",
    fields=[
...
node_group_resource = ItemDynamicLayout.set_fields(
    "Resources",
    fields=[
        ListDyField.data_source(
            "Auto Scaling Group Name",
            "data.resources.auto_scaling_groups",
            options={"delimiter": "<br>", "sub_key": "name"},
        ),
        ListDyField.data_source(
            "Auto Scaling Group ARN",
            "data.resources.auto_scaling_groups",
            options={"delimiter": "<br>", "sub_key": "arn"},
        ),
        TextDyField.data_source(
            "Remote Access Security Group",
            "data.resources.remote_access_security_group",
        ),
    ],
)

node_group_health = TableDynamicLayout.set_fields(
    "Health",
    "data.health.issues",
    fields=[
...
node_group_metadata = CloudServiceMeta.set_layouts(
    layouts=[
        node_group_base,
...
class NodeGroupResource(EKSResource):
    ...
    _metadata = ModelType(
        CloudServiceMeta, default=node_group_metadata, serialized_name="metadata"
    )
