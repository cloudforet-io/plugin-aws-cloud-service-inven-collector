...
from spaceone.inventory.connector.aws_documentdb_connector.schema.data import Cluster, SubnetGroup, ParameterGroup
...
# TAB - BASE
meta_base = ItemDynamicLayout.set_fields('Clusters', fields=[
    ...
    TextDyField.data_source('Cluster Parameter group', 'data.db_cluster_parameter_group'),
    EnumDyField.data_source('Deletion Protection', 'data.deletion_protection', default_badge={
        'indigo.500': ['true'], 'coral.600': ['false']
    }),
    ListDyField.data_source('Enabled Cloudwatch Logs Exports', 'data.enabled_cloudwatch_logs_exports',
...
cluster_sg = TableDynamicLayout.set_fields('Security Groups', 'data.vpc_security_groups', fields=[
...
instances = TableDynamicLayout.set_fields('Instances', 'data.instances', fields=[
    TextDyField.data_source('Instance', 'db_instance_identifier'),
    EnumDyField.data_source('Status', 'db_instance_status', default_state={
        'safe': ['available']
    }),
    ...
    ListDyField.data_source('Security Groups', 'vpc_security_groups', default_badge={
        'type': 'outline',
...
snapshots = TableDynamicLayout.set_fields('Snapshots', 'data.snapshots', fields=[
    ...
    EnumDyField.data_source('Type', 'snapshot_type', default_outline_badge=['automated', 'manual', 'shared', 'public']),
    EnumDyField.data_source('Encrypted', 'storage_encrypted', default_badge={
        'indigo.500': ['true'], 'coral.600': ['false']
    }),
    DateTimeDyField.data_source('Creation Time', 'snapshot_create_time'),
])

cluster_subnet_groups = ItemDynamicLayout.set_fields('Subnet Groups', 'data.subnet_group', fields=[
    TextDyField.data_source('Name', 'db_subnet_group_name'),
    TextDyField.data_source('ARN', 'db_subnet_group_arn'),
    EnumDyField.data_source('Status', 'subnet_group_status', default_state={
        'safe': ['Complete']
    }),
...
cluster_parameter = TableDynamicLayout.set_fields('Parameters', 'data.parameter_group.parameters', fields=[
...
subnet = TableDynamicLayout.set_fields('Subnets', 'data.subnets', fields=[
    ...
    EnumDyField.data_source('Subnet Group Status', 'subnet_status', default_state={
        'safe': ['Active']
    }),
])
...
parameter = TableDynamicLayout.set_fields('Parameters', 'data.parameters', fields=[
    TextDyField.data_source('Name', 'parameter_name'),
    TextDyField.data_source('Value', 'parameter_value'),
    ...
    EnumDyField.data_source('Apply Type', 'apply_type', default_outline_badge=['static', 'dynamic']),
...
class SubnetGroupResource(DocumentDBResource):
    ...
    _metadata = ModelType(CloudServiceMeta, default=subnet_group_metadata, serialized_name='metadata')
...
class ParameterGroupResource(DocumentDBResource):
    ...
    _metadata = ModelType(CloudServiceMeta, default=parameter_group_metadata, serialized_name='metadata')
