...
from spaceone.inventory.connector.aws_auto_scaling_connector.schema.data import (
    AutoScalingGroup,
    LaunchConfiguration,
    AutoScalingPolicy,
    LifecycleHook,
...
from spaceone.inventory.connector.aws_auto_scaling_connector.schema.resource import (
...
from spaceone.inventory.connector.aws_auto_scaling_connector.schema.service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class AutoScalingConnector(SchematicAWSConnector):
    ...
    def get_resources(self):
        _LOGGER.debug(
            f"[get_resources][account_id: {self.account_id}] START: Auto Scaling"
        )
        resources = []
                ...
                try:
                    ...
                    if notification_configurations is None:
                        notification_configurations = (
                            self._describe_notification_configurations()
                        )
                    ...
                    raw.update(
                        {
                            "launch_configuration": LaunchConfiguration(
                                match_lc, strict=False
                            ),
                            "policies": list(
                                map(
                                    lambda policy: AutoScalingPolicy(
                                        policy, strict=False
                                    ),
                                    match_policies,
                                )
                            ...
                            "notification_configurations": list(
                                map(
                                    lambda noti_conf: NotificationConfiguration(
                                        noti_conf, strict=False
                                    ),
                                    match_noti_confs,
                                )
                            ),
                            "scheduled_actions": list(
                                map(
                                    lambda scheduled_action: ScheduledAction(
                                        scheduled_action, strict=False
                                    ),
                            ...
                            "lifecycle_hooks": list(
                                map(
                                    lambda lifecycle_hook: LifecycleHook(
                                        lifecycle_hook, strict=False
                                    ),
                                    self._describe_lifecycle_hooks(
                                        raw["AutoScalingGroupName"]
                                    ),
                                )
                            ),
                            "autoscaling_tags": list(
                                map(
                                    lambda tag: AutoScalingGroupTags(tag, strict=False),
                                    raw.get("Tags", []),
                                )
                            ),
                    ...
                    yield {
                        ...
                        "launched_at": self.datetime_to_iso8601(
                            auto_scaling_group_vo.created_time
                        ),
                    }
    ...
    def request_launch_configuration_data(
        self, region_name
            ...
            for raw in data.get("LaunchConfigurations", []):
                try:
                    ...
                    yield {
                        ...
                        "account": self.account_id,
                        "launched_at": self.datetime_to_iso8601(
                            launch_configuration_vo.created_time
                        ),
                    }
            ...
            for raw in data.get("LaunchTemplates", []):
                try:
                    ...
                    yield {
                        ...
                        "tags": self.convert_tags_to_dict_type(raw.get("Tags", [])),
                        "launched_at": self.datetime_to_iso8601(
                            launch_template_vo.create_time
                        ),
                    }
    ...
    def get_load_balancer_info(self, lb_arns):
        ...
        for lb_arns in split_lb_arns:
            try:
                lbs = elb_client.describe_load_balancers(LoadBalancerArns=lb_arns).get(
                    "LoadBalancers", []
                )
    ...
    def _match_launch_template(self, raw):
        ...
        return next(
            (
                ...
                if launch_template.launch_template_id == lt_dict.get("LaunchTemplateId")
            ),
            None,
        )
